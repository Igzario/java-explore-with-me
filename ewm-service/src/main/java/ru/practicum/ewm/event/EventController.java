...
public class EventController {
    ...
    @GetMapping("/{eventId}")
    @ResponseStatus(HttpStatus.OK)
    EventFullDto findEventById(@PathVariable Long eventId, HttpServletRequest request) {
        log.info("Admin: запрос на вывод события с ID-: {}", eventId);
        saveHit(request);
    ...
    private void saveHit(HttpServletRequest request) {
        hitClient.addHit(EndPointHit.builder()
                ...
                .build());
        log.info("Admin: запрос отправлен на сервер статистики: {}", request);
    }
}